<%@ Assembly Name="FloorPlan, Version=1.0.0.0, Culture=neutral, PublicKeyToken=2c1e91d1db271485" %>
    ...
    function onShapeSelectionChanged(source, args) {
        ...
        if (shape.getName().substring(0, 5) == 'Space') {
            ...
            if (shapeData[0].value == 'Office') {
                ...
                Microsoft.Office.Server.UserProfiles.UserProfileService.GetUserProfileByName
                    ~(shapeData[6].value, onProfileRequestSuccess, onProfileRequestFailed);
            }
        ...
        else document.getElementById('roomInfo').className = 'hidden';
    }

    function onProfileRequestSuccess(results) {
        document.getElementById('userPhoto').src =
        ~results[14].Values == null ? "" : results[14].Values[0].Value;
        document.getElementById('userName').innerText =
        ~results[6].Values == null ? "" : results[6].Values[0].Value;
        document.getElementById('userPhone').innerText =
        ~results[8].Values == null ? "" : results[8].Values[0].Value;
